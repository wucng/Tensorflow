...
class Input(object):
    ...
    def split_train_and_test(self):
        self.data = create_pickle_train2(self.image_path, self.mask_path, img_pixel, channels)
...
def model():
    if train == 1:
        ...
        decay_steps = int(num_batches_per_epoch * 2)  # 每2个epoch 衰减学习率
        # decay_steps = int(2)
    ...
    def conv_net(x, dropout):
        ...
        x = tf.reshape(x, shape=[-1, img_pixel, img_pixel, channels])  # [batch,400,400,3]
        ...
        out_end_1 = tf.nn.conv2d_transpose(out_1,
                                           tf.get_variable("up_sample_1", shape=[4, 4, label_cols, label_cols],
                                                           initializer=tf.random_uniform_initializer()),
                                           output_shape=[batch_size, img_pixel // 16, img_pixel // 16,
                                                         label_cols],
                                           strides=[1, 2, 2, 1],
                                           padding="SAME")  # [batch_size,img_pixel//16,img_pixel//16,label_cols] 即[batch,25,25,3]
        ...
        out_end_2 = tf.nn.conv2d_transpose(out_upsample_pool4,
                                           tf.get_variable("up_sample_4", shape=[4, 4, label_cols, label_cols],
                                                           initializer=tf.random_uniform_initializer()),
                                           output_shape=[batch_size, img_pixel // 8, img_pixel // 8,
                                                         label_cols],
                                           strides=[1, 2, 2, 1],
                                           padding='SAME')  # [batch_size,img_pixel//8,img_pixel//8,label_cols] 即 [batch,50,50,3]
        # 让pool_4卷积输出为第一维为labcolS大小，并和out_end_1做融合，2017.10.12,by xjxf  __start
        ...
        out_end_3 = tf.nn.conv2d_transpose(out_upsample_pool3,
                                           tf.get_variable("up_sample_3", shape=[4, 4, label_cols, label_cols],
                                                           initializer=tf.random_uniform_initializer()),
                                           output_shape=[batch_size, img_pixel // 4, img_pixel // 4,
                                                         label_cols],
                                           strides=[1, 2, 2, 1],
                                           padding='SAME')  # [batch_size, img_pixel//4, img_pixel//4, label_cols] 即 [batch,100,100,3]
        # 让pool_3卷积输出为第一维为labcolS大小，并和out_end_1做融合，2017.10.12,by xjxf  __start
        ...
        out_end_4 = tf.nn.conv2d_transpose(out_upsample_pool2,
                                           tf.get_variable("up_sample_2", shape=[8, 8, label_cols, label_cols],
                                                           initializer=tf.random_uniform_initializer()),
                                           output_shape=[batch_size, img_pixel, img_pixel, label_cols],
                                           strides=[1, 4, 4, 1],
                                           padding='SAME')  # [batch_size, img_pixel, img_pixel, label_cols] 即 [batch,400,400,3]
        # 让pool_3卷积输出为第一维为labcolS大小，并和out_end_1做融合，2017.10.16,by xjxf  __start
...
cost, pred, optimizer, accuracy = model()
...
def Train(sess):
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            img, label = next_batch(data, batch_size, flag, img_pixel=img_pixel, channels=channels)
            flag = 0
    ...
    for dir_name in img_paths:
        ...
        for i in range(YSize1, YSize2, isizes):
            ...
            for j in range(0, srcXSize, isizes):
                ...
                data2 = data_1[i:i + img_pixel, j:j + img_pixel].reshape([-1, img_size])
            ...
            pred_y = sess.run(pred, feed_dict={x: data_2_1, keep_prob: 1.})
            # print(pred_y.shape);exit(-1)
            for row, pred1 in enumerate(pred_y):
                if np.argmax(pred1):
                    # print('pred3', pred3)
                    jj = row * img_pixel
                    if jj + img_pixel >= srcXSize - 1:
                        jj = srcXSize - 1 - img_pixel
                    dBuf_1[i:i + img_pixel, jj:jj + img_pixel] = np.ones([img_pixel, img_pixel], np.uint8) * 255
                    # """
...
# img [1,400*400*3]
# label [400*400,3]
def Inference2(sess, task_index):
    # img_paths = ['gtdata:///users/xiaoshaolin/tensorflow/train_data/train_img_01.tif',
    #              'gtdata:///users/xiaoshaolin/tensorflow/train_data/train_img_02.tif']
    # img_paths=['HDFS://dm01-08-01.tjidc.dcos.com:8020/nanlin/china_q3/L15-1792E-1340N.tif',
    #            'HDFS://dm01-08-01.tjidc.dcos.com:8020/nanlin/china_q3/L15-1792E-1339N.tif']
    ...
    # [img_paths.append('HDFS://dm01-08-01.tjidc.dcos.com:8020/nanlin/china_q3/' + i.replace('\n', ''))
    #  for i in open('./tif.log')]  # 从hdfs上读取文件tile.log
    ...
    for dir_name in img_paths[:10]:
        ...
        flagY = True
        for i in range(0, srcYSize, img_pixel):
            if not flagY:
                break
            if i + img_pixel > srcYSize:
                i_end = srcYSize
                flagY = False
            else:
                i_end = i + img_pixel
            ...
            flagX = True
            for j in range(0, srcXSize, img_pixel):
                if not flagX:
                    break
                if j + img_pixel > srcXSize:
                    j_end = srcXSize
                    flagX = False
                else:
                    j_end = j + img_pixel
                    ...
                    # Read raster as arrays 类似RasterIO（C++）
                    # dataraster = srcband.ReadAsArray(j, i, img_pixel, img_pixel, img_pixel, img_pixel)

                dataraster = (data_1[i:i_end, j:j_end])
                dataraster = dataraster * (1. / 255) - 0.5
                dataraster_new = np.lib.pad(dataraster,
                                            ((0, img_pixel - (i_end - i)), (0, img_pixel - (j_end - j)), (0, 0)),
                ...
                mask_value = value_1.reshape([img_pixel, img_pixel])
...
def Inference3(task_index):
    ...
    # for dir_name in img_paths[task_index:task_index + 1]:
    dir_name= img_paths[task_index]
    srcDS = gdal.Open(dir_name, GA_ReadOnly)  # 只读方式打开原始影像
...
if train == -1:
    ...
    print(list(map(Inference3,range(2)))[0])
