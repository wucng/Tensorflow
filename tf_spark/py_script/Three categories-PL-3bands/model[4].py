...
parser.add_argument('-is','--img_size', type=int, default=400,help='image size')
parser.add_argument('-ic','--img_channel', type=int, default=3,help='image channel')
parser.add_argument('-n','--n_class', type=int, default=3,help='Sample classification number')
parser.add_argument('-e','--training_epochs', type=int, default=2,help='training epochs')
parser.add_argument('-b','--batch_size', type=int, default=1,help='batch size')
parser.add_argument('-ds','--display_step', type=int, default=1,help='display step')
parser.add_argument('-dr','--dropout', type=float, default=0.8,help='dropout')
parser.add_argument('-tr','--train', type=int, default=-1,choices=[1, -1],help='1 tarin ,-1 inference')

parser.add_argument('-si','--start_index', type=int, default=0,help='Index of start within img')
parser.add_argument('-ei','--end_index', type=int, default=3717,help='Index of end within img')
...
train = args.train  # 1 tarin ;-1 inference
...
def Inference(sess,task_index):
    ...
    img_paths = ['./traindata/19.tif','./traindata/20.tif','./traindata/21.tif']
    ...
    end = min(args.end_index, len(img_paths))
    for ix, dir_name in enumerate(img_paths[args.start_index:end]):
...
if __name__=="__main__":
    ...
    if train==-1:
        ...
        # saver.restore(sess, 'hdfs://dm01-08-01.tjidc.dcos.com:8020/user/root/model/save_net.ckpt') # 直接从hdfs上提取模型
        Inference(sess,0)
    sess.close()
