...
一台机上同时执行ps与worker
同一台机 打开2个终端执行：
python mnist_dist.py --ps_hosts=10.0.100.15:2220 --worker_hosts=10.0.100.15:2220 --job_name="ps" --task_index=0
python mnist_dist.py --ps_hosts=10.0.100.15:2220 --worker_hosts=10.0.100.15:2220 --job_name="worker" --task_index=0
或
python mnist_dist.py --ps_hosts=10.0.100.15:2220 --worker_hosts=10.0.100.15:2221 --job_name="ps" --task_index=0 --start_index=0 --num_images=182
python mnist_dist.py --ps_hosts=10.0.100.15:2220 --worker_hosts=10.0.100.15:2221 --job_name="worker" --task_index=0 --start_index=0 --num_images=182

# 或
nohup python mnist_dist.py --ps_hosts=10.0.100.14:2220 --worker_hosts=10.0.100.14:2221 --job_name="ps" --task_index=0 --start_index=0 --num_images=182 &

nohup python mnist_dist.py --ps_hosts=10.0.100.14:2220 --worker_hosts=10.0.100.14:2221 --job_name="worker" --task_index=0 --start_index=0 --num_images=182 &
--------------集群版----------------
直接从gtdata/hdfs上读取图像与对应的掩膜图像，生成图像数据+标签数据 卷积模型
执行命令：
spark-submit
--queue default \
--num-executors 45 \
...
tf.app.flags.DEFINE_string("ps_hosts", "10.0.100.15:2220", "Comma-separated list of hostname:port pairs")
tf.app.flags.DEFINE_string("worker_hosts", "10.0.100.15:2221", "Comma-separated list of hostname:port pairs")
tf.app.flags.DEFINE_string("job_name", "", "One of 'ps', 'worker'")
tf.app.flags.DEFINE_integer("task_index", 0, "Index of task within the job")
tf.app.flags.DEFINE_integer("start_index", 0, "Image start index")
tf.app.flags.DEFINE_integer("num_images", 182, "The number of images processed each time")
    ...
    if FLAGS.job_name == "ps":
        ...
        if tarin==-1: # inference
            while not sv.should_stop():
                ...
                print('平均时间：',(end_time - start_time).seconds/FLAGS.num_images)
                sess.close()
                exit()
