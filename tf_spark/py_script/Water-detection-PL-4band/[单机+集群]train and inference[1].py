...
class Train_and_inference_model(object):
    ...
    def Train(self):
        [cost, pred, optimizer, accuracy] =Model(train).model()
        self.sess.run(tf.global_variables_initializer())
...
if __name__=="__main__":
    ...
    # sess.run(init)
    if train==1:
        Train_and_inference_model(sess).Train()

    if train==-1:
        Train_and_inference_model(sess).Inference()
