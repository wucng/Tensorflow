...
class Model(object):
    def __init__(self,train,sess):
        # tf Graph Input
        self.train=train
        self.sess=sess
        self.img_nums=Input().split_train_and_test()[-1]
    ...
    def model(self):
        if self.train==1:
            ...
            num_batches_per_epoch = int(self.img_nums / batch_size)
    ...
    def Train(self):
        [cost,pred,optimizer,accuracy]=self.model()
        data=Input().split_train_and_test()[0]
        total_batch = int(self.img_nums / batch_size)
        ...
        for epoch in range(training_epochs):
            ...
            for i in range(total_batch):
                ...
                _, c, p, gl_step = self.sess.run([optimizer, cost, pred, global_step], feed_dict={x: batch_xs,
                                                                                             y: batch_ys,
                                                                                             keep_prob: dropout})
            ...
            if (epoch + 1) % display_step == 0:
                ...
                print("Epoch:", '%04d' % (epoch + 1),
                      ...
                      'accuracy:', self.sess.run(accuracy, feed_dict={x: batch_xs, y: batch_ys, keep_prob: 1.0}))
        ...
        for dir_name in img_paths:
            ...
            def compute(YSize1, YSize2):  # 每次计算YSize1~YSize2 行，实现伪并行计算
                ...
                for i in range(YSize1, YSize2, isizes):
                    ...
                    for row, pred1 in enumerate(self.sess.run(pred, feed_dict={x: data_2_1, keep_prob: 1.})):
                        if np.argmax(pred1):
                            # print('pred3', pred3)
                            jj = row * isize
