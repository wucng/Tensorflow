...
def create_pickle_train(image_path, mask_path, pkl_path, img_pixel=1, channels=3):
    m = 0
    n=0
    ...
    data_list = []
    flag=True
    for i in range(0, x_size - img_pixel + 1,tt):  # 文件夹下的文件名
        if not flag:break
        if i + img_pixel > x_size:
            i = x_size - img_pixel - 1
        for j in range(0, y_size - img_pixel + 1,tt):
