...
使用GDAL读取影像
存储成pickle
取对应image的img_pixel x img_pixel大小的掩膜作为标签（生成任意像素大小的样本）
"""
import numpy as np
import pickle
import os
...
img_pixel=10 # 适用于所有像素样本

def create_pickle_train(image_path, mask_path, pkl_path, img_pixel, channels=3):
    m = 0
    n=0
    ...
    t=img_pixel // 2
    if t==0:t=1
    for i in range(0, x_size - img_pixel + 1,t):  # 文件夹下的文件名
        if not flag:break
        if i + img_pixel > x_size:
            i = x_size - img_pixel - 1
        for j in range(0, y_size - img_pixel + 1,t):
            ...
            train_label = mask_data[i:i + img_pixel, j:j + img_pixel] #.max()  img_pixel x img_pixel 对应标签
            train_label=np.reshape(train_label,[-1,img_pixel * img_pixel])
            # train_label = 1
...
if __name__ == "__main__":
    ...
    for i in range(len(data_images)):
        ...
        create_pickle_train(image_path, mask_path, pkl_path, img_pixel=img_pixel, channels=3)
