...
def create_pickle_train(image_path, mask_path, img_pixel=10, channels=3):
    ...
    for i in range(0, x_size - img_pixel + 1, img_pixel // 2):  # 文件夹下的文件名
        ...
        for j in range(0, y_size - img_pixel + 1, img_pixel // 2):
            ...
            train_label = mask_data[i:i + img_pixel, j:j + img_pixel]
            train_label=train_label.reshape(-1,img_pixel * img_pixel)
            # train_label = 1
    ...
    data_matrix = data_matrix.reshape((-1, img_pixel*img_pixel*channels+img_pixel * img_pixel))
    return data_matrix
...
def writeMNIST(sc, dir_name,dir_name1,img_pixel,channels, output, format, num_class,num_partitions): # 按照自己数据格式相应修改
  '''
  ...
  :param num_class:    实际图像分类数  mnist 10分类 所有为10
  :param num_partitions:  最后生成数据的分区数
  :return: HDFS
  '''
  ...
  labels_dense=data[:,img_pixel * img_pixel * channels:] #取出标签列
  if format == "csv2":  # 数据格式
    labels=labels_dense
  else:
    # 转成one_hot
    # labels=dense_to_one_hot2(labels_dense,num_class)
    labels = labels_dense
  del labels_dense
...
if __name__ == "__main__":
  ...
  parser.add_argument("-i", "--imgPixel", help="The image pixel size", type=int,default=16)
  parser.add_argument("-c", "--channels", help="The channel of the image", type=int, default=3)
  parser.add_argument("-nc", "--num_class", help="The class of the image", type=int, default=2)
  parser.add_argument("-n", "--num-partitions", help="Number of output partitions", type=int, default=10) # 数据分成几个分区,如10，最后生成的数据将被分成10个部分
  ...
  if not args.read: # 写入
    ...
    writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/11.tif', 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/11_mask.tif',
             args.imgPixel, args.channels, args.output + "/11", args.format,args.num_class,
             args.num_partitions)  # 转换train数据  # 从gtdata上读取数据
    # writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/12.tif', 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/12_mask.tif',
    #           args.imgPixel, args.channels, args.output + "/12", args.format,args.num_class,
    #           args.num_partitions)  # 转换train数据  # 从gtdata上读取数据
    #writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/13.tif', 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/13_mask.tif',
    #          args.imgPixel, args.channels, args.output + "/13", args.format,args.num_class,
    #          args.num_partitions)  # 转换train数据  # 从gtdata上读取数据
    #writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/14.tif', 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/14_mask.tif',
    #          args.imgPixel, args.channels, args.output + "/14", args.format,args.num_class,
    #          args.num_partitions)  # 转换train数据  # 从gtdata上读取数据
    #writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/33.tif', 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/33_mask.tif',
    #          args.imgPixel, args.channels, args.output + "/33", args.format,args.num_class,
    #          args.num_partitions)  # 转换train数据  # 从gtdata上读取数据
    #writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/34.tif', 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/34_mask.tif',
    #          args.imgPixel, args.channels, args.output + "/34", args.format,args.num_class,
    #          args.num_partitions)  # 转换train数据  # 从gtdata上读取数据
    #writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/35.tif', 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/35_mask.tif',
    #          args.imgPixel, args.channels, args.output + "/35", args.format,args.num_class,
    #          args.num_partitions)  # 转换train数据  # 从gtdata上读取数据
    #writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/36.tif', 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/36_mask.tif',
    #          args.imgPixel, args.channels, args.output + "/36", args.format,args.num_class,
    #          args.num_partitions)  # 转换train数据  # 从gtdata上读取数据
    #writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/37.tif', 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/37_mask.tif',
    #          args.imgPixel, args.channels, args.output + "/37", args.format,args.num_class,
    #          args.num_partitions)  # 转换train数据  # 从gtdata上读取数据
    #writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/38.tif', 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/38_mask.tif',
    #          args.imgPixel, args.channels, args.output + "/38", args.format,args.num_class,
    #          args.num_partitions)  # 转换train数据  # 从gtdata上读取数据
