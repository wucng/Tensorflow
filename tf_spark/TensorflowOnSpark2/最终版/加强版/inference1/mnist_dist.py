...
"""
  ...
  def feed_dict(batch):
    ...
    xs=(xs - numpy.min(xs, 0)) / (numpy.max(xs, 0) - numpy.min(xs, 0)+0.0001)
  ...
  if job_name == "ps":
    ...
    with tf.device(tf.train.replica_device_setter(
      ...
      if args.mode != 'inference':
        ...
        #                                            0.96,
        #                                            staircase=False)
        learning_rate=tf.train.polynomial_decay(INITIAL_LEARNING_RATE,global_step,3000000,1e-5,0.5,True)
      ...
      while not sv.should_stop() and not tf_feed.should_stop() and step < args.steps:
        ...
        if len(batch_xs) > 0:
          if args.mode == "train" or args.mode == "retrain":
            ...
            if (step % 100 == 0):
              print("{0} step: {1} accuracy: {2}".format(datetime.now().isoformat(), step, sess.run(accuracy,{x: batch_xs, y_: batch_ys,keep:1.,is_training:False})))
              # log.info("{0} step: {1} accuracy: {2}".format(datetime.now().isoformat(), step, sess.run(accuracy,{x: batch_xs, y_: batch_ys})))
            if sv.is_chief:
              pass
              # summary_writer.add_summary(summary, step)
          else: # args.mode == "inference"
            feed2 = {x: batch_xs, y_: batch_ys, keep: 1., is_training: False}
            # labels, preds, acc = sess.run([label, prediction, accuracy], feed_dict=feed2)
            labels, preds= sess.run([label, prediction], feed_dict=feed2)
