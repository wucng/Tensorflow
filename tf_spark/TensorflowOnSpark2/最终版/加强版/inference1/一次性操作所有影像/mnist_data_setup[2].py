...
可以一次性转换出所有文件  tile.log 存放所有影像的文件名
...
# ---------------------------------------------------
# inference
def create_pickle_train2(image_path, img_pixel=2, channels=3):
    # m = 0
    # image_data = Multiband2Array(image_path)
    image_data=Multiband2Array(image_path)
    # mask_data = cv2.split(cv2.imread(mask_path))[0] / 255
...
def writeMNIST(sc, dir_name,dir_name1,img_pixel,channels, output, format, num_class,num_partitions,mode): # 按照自己数据格式相应修改
  '''
  ...
  if mode==0 or mode==1: # train or test
    ...
    data=create_pickle_train2(dir_name,img_pixel,channels)
...
if __name__ == "__main__":
  ...
  if not args.read: # 写入
    ...
    # file_name=tf.gfile.Glob('gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/*.tif')

    file_name = []
    [file_name.append('gtdata:////users/xiaoshaolin/tensorflow/hubei/'+i.replace('\n',''))
     for i in open('mnist/tile.log')] # 从hdfs上读取文件tile.log

    # file_name=['gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/11.tif','gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/12.tif']
    [writeMNIST(sc, file, # 从gtdate上读取影像
               '',
               args.imgPixel, args.channels, args.output + "/"+file.split('/')[-1].split('.')[0], args.format, args.num_class,
               args.num_partitions,args.mode) for file in file_name]

    # writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/13.tif',
    #            '',
