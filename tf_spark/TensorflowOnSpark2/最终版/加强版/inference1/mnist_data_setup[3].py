...
# return： 图像数组
'''
def Multiband2Array(path,im,img_sem):
    ...
    rows=ycount//img_sem  # 将图像行分成img_sem个部分，即每次只读取rows行数据
    ...
    for band in range(ibands):
        ...
        if im==img_sem-1:
            dataraster = srcband.ReadAsArray(0, im * rows, xcount, ycount-rows*im).astype(np.float32)
            if band == 1:
                data = dataraster.reshape((xcount, ycount-rows*im, 1))
            else:
                # 将每个波段的数组很并到一个3维数组中
                data = np.append(data, dataraster.reshape((xcount, ycount-rows*im, 1)), axis=2)
        else:
            dataraster = srcband.ReadAsArray(0, im*rows, xcount, rows).astype(np.float32)
            if band == 1:
                data = dataraster.reshape((xcount, rows, 1))
            else:
                # 将每个波段的数组很并到一个3维数组中
                data = np.append(data, dataraster.reshape((xcount, rows, 1)), axis=2)



    return data,xcount,ycount
...
# ---------------------------------------------------
# inference
def create_pickle_train2(image_path, img_pixel, channels,im,img_sem):
    # m = 0
    # image_data = Multiband2Array(image_path)
    image_data,xcount,ycount=Multiband2Array(image_path,im,img_sem)
    # mask_data = cv2.split(cv2.imread(mask_path))[0] / 255
    # mask_data=np.asarray(Image.open(mask_path))//255
    ...
    for i in range(0, x_size - img_pixel + 1, img_pixel):  # 文件夹下的文件名
        ...
        for j in range(0, y_size - img_pixel + 1, img_pixel):
            ...
            # train_label=[i,j,x_size,y_size]
            # train_label = [i, j, xcount, ycount]
            train_label = [i, j, xcount, ycount,im,img_sem]

            # train_label = str(i)+'-'+str(j)+'-'+str(x_size)+'-'+str(y_size)
            data2 = np.append(data1, train_label)[np.newaxis, :]  # 数据+标签
            data_list.append(data2)
    ...
    data_matrix = data_matrix.reshape((-1, img_pixel*img_pixel*channels+4))
    return data_matrix
...
def writeMNIST(sc, dir_name,dir_name1,img_pixel,channels, output, format, num_class,num_partitions,mode,im,img_sem): # 按照自己数据格式相应修改
  '''
  ...
  :param num_partitions:  最后生成数据的分区数，即分成几个部分
  :param mode:  train 1,test 0,inference -1
  :return: HDFS
  '''
  ...
  if mode==0 or mode==1: # train or test
    data=create_pickle_train(dir_name,dir_name1,img_pixel,channels)
  else: # inference mode==-1
    data=create_pickle_train2(dir_name,img_pixel,channels,im,img_sem)
...
if __name__ == "__main__":
  ...
  parser.add_argument("-i", "--imgPixel", help="The image pixel size", type=int,default=2)
  ...
  parser.add_argument("-m", "--mode", help="train|test|inference", type=int, default=-1) # train 1,test 0,inference -1

  parser.add_argument("-im", "--img_seg_num", help="Image segmentation part number", type=int, default=2000) # 图像按行分成多少份
  ...
  if not args.read: # 写入
    ...
    # writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/GF/PART2/11.tif', 'gtdata:///users/xiaoshaolin/tensorflow/11_mask.tif',
    #           args.imgPixel, args.channels, args.output + "/train", args.format,args.num_class,
    #           args.num_partitions)  # 转换train数据  # 从gtdata上读取数据


    [writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/hubei/whole/Mosaic_hubei.tif',
               '',
               args.imgPixel, args.channels, args.output + "/"+str(im), args.format, args.num_class,
               args.num_partitions,args.mode,im,args.img_seg_num) for im in range(args.img_seg_num)]


    # writeMNIST(sc, 'gtdata:///users/xiaoshaolin/tensorflow/hubei/whole/Mosaic_hubei.tif',
    #            '',
    #            args.imgPixel, args.channels, args.output + "/Mosaic", args.format, args.num_class,
    #            args.num_partitions,args.mode)  # 转换train数据  # 从gtdata上读取数据

    # writeMNIST(sc, 'mnist/11.tif', 'mnist/11_mask.tif',
    #            args.imgPixel, args.channels, args.output + "/train", args.format,args.num_class,
    #            args.num_partitions)  # 转换train数据 # 从hdfs上读取数据
    # writeMNIST(sc, 'mnist/33.pkl', args.imgPixel, args.channels, args.output + "/train6", args.format,args.num_class,
    #            args.num_partitions)  # 转换train数据 # 从hdfs上读取数据
    # writeMNIST(sc, 'mnist/34_0.pkl', args.imgPixel, args.channels, args.output + "/train7", args.format,args.num_class,
    #            args.num_partitions)  # 转换train数据 # 从hdfs上读取数据
    #writeMNIST(sc, 'mnist/12.pkl', args.imgPixel, args.channels, args.output + "/test", args.format,args.num_class,
    #           args.num_partitions)  # 转换test数据 # 从hdfs上读取数据
