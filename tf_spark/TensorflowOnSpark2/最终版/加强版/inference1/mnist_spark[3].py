...
parser.add_argument("-s", "--steps", help="maximum number of steps", type=int, default=200000) # 总的训练步数
...
parser.add_argument("-nu", "--num", help="number of inference iamges", type=int, default=0) # 样本推理次数
...
if args.num<len(file_name):
  start=args.num
  stop=min(args.num+6,len(file_name))
  # if stop>len(file_name):stop=len(file_name)
  for fileN in file_name[start:stop]:
    if args.format == "tfr":  # HDFS==>numpy array
      ...
      if args.format == "csv": # HDFS==>numpy array
        ...
        images = sc.pickleFile(args.images+'/'+fileN+'/images')
        labels = sc.pickleFile(args.labels+'/'+fileN+'/labels')
    ...
    if args.mode == "train" or args.mode == "retrain":
      ...
      labelRDD.saveAsTextFile(args.output+'/'+fileN)
