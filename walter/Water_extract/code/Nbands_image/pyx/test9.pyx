...
cdef int isize=10 # 10 x 10的样本
cdef int m=1
cdef int isizes=m*isize
...
cdef int srcXSize = srcDS.RasterXSize  # 宽度
cdef int srcYSize = srcDS.RasterYSize  # 高度
cdef int nbands = srcDS.RasterCount  # 波段数

# raster_fn = path.join(dir_name, 'test_mask.tiff')  # 存放掩膜影像
raster_fn=dir_name+"test_mask.tiff"
        ...
        for j in range(0,srcXSize,isizes):
            ...
            if sess.run(tf.argmax(pred2, 1))[0]:
                ...
                for xx in range(isize):
                    # xx=int(xx)
                    # print('loop2 start:', datetime.datetime.now())
                    for yy in range(isize):
                        # y=int(y)
                        image3 = Image.fromarray(np.uint8(data[xx:xx+1, yy:yy+1, :]))
                        ...
                        if sess.run(tf.argmax(pred3, 1))[0]:
                            dBuf_1[i:i+isize,j:j+isize][tt*xx:tt*xx+tt, tt*yy:tt*yy+tt] = np.ones((tt,tt),np.uint8)*255
                        del pred3
